...
class StreamViewer:
        ...
        try:
            ...
            if os.path.exists(rules_file):
                ...
                if not self.has_authorization_rules:
                    logger.info(
                        f"No active authorization rules found for role: {self.authorized_role} and camera: {self.camera_id}")
                ...
                try:
                    ...
                    if faces:
                        ...
                        if best_face is not None:
                            ...
                            logger.warning(
                                f"No good face detected for {username}, using full image with moderate quality")
                    else:
    ...
    def _compare_with_insightface(self, face_img, reference_data):
        ...
        try:
            ...
            similarity = np.dot(ref_embedding, test_embedding) / (
    ...
    def _is_authorized_person(self, person_img, person_id=None):
        ...
        for username, ref_data in self.reference_images.items():
            ...
            if is_insightface_detection and HAVE_INSIGHTFACE and self.insightface_analyzer:
                ...
                try:
                    is_match, similarity = self.yolo_model.compare_with_reference(best_face, ref_img,
    ...
    def start(self):
        ...
        # 修改部分开始：检查是否为本地摄像头索引（如 "0"）
        if self.rtsp_url and self.rtsp_url.isdigit():
            # 这是本地摄像头索引，跳过RTSP验证
            logger.info(f"Using local camera index {self.rtsp_url}, skipping RTSP validation")
            stream_valid = {"is_valid": True, "frames_read": 1, "validation_time": 0.1}
        else:
            # 这是真正的RTSP流，进行验证
        ...
        # 对于本地摄像头，不要添加RTSP选项
        if self.rtsp_url and not self.rtsp_url.isdigit():
        ...
        # 修改部分开始：根据是否为本地摄像头选择不同的打开方式
        if self.rtsp_url and self.rtsp_url.isdigit():
            # 本地摄像头：直接使用索引
            camera_index = int(self.rtsp_url)
            logger.info(f"Opening local camera with index: {camera_index}")
            cap = cv2.VideoCapture(camera_index)
        else:
            # RTSP流：使用FFMPEG后端
            logger.info(f"Opening RTSP stream: {rtsp_url_with_options}")
            cap = cv2.VideoCapture(rtsp_url_with_options, cv2.CAP_FFMPEG)
        # 修改部分结束

        if not cap.isOpened():
            logger.error(f"Could not open stream: {self.rtsp_url}")
            return False
        ...
        try:
            while True:
                ...
                if not ret:
                    ...
                    logger.warning(
                        f"Failed to read frame from stream (error {consecutive_errors}/{max_consecutive_errors})")
...
if __name__ == "__main__":
    import sys
    # 如果直接在PyCharm中运行（没有命令行参数）
    if len(sys.argv) == 1:
        # 添加默认参数
        sys.argv = [
            'stream_viewer.py',
            '--camera-id', '19b14fd5-6fe6-4cc1-8af2-f78ead10a818',
            '--role', 'admin'
        ]
    main()